...
public class InputThread extends Thread {
	...
	public void readMessage() throws IOException, ClassNotFoundException {
		...
		System.out.println("接受客户端消息"+readObject);
		...
		if (readObject != null && readObject instanceof TranObject) {
			...
			case LOGIN:
				...
				break;
			case FriendCheck:
				User Friend = (User) read_tranObject.getObject();
				// 给用户回复消息
				TranObject<User> register3TranObject = new TranObject<User>(
						TranObjectType.FriendCheck);
				register3TranObject.setObject(dao.findFriend(Friend.getId()));
				out.setMessage(register3TranObject);
				break;
		  	case FriendAdd:
				// 给用户回复消息
		  		User user55 = (User) read_tranObject.getObject();
				dao.AddFriend(user55 ,read_tranObject.getFromUser());
			//	User logoutUser = (User) read_tranObject.getObject();

				TranObject<User> offObject1 = new TranObject<User>(
						TranObjectType.AddFriendMsg);
				User user56 = new User();
				user56.setId(user55.getId());
				offObject1.setObject(user56);
				for (OutputThread offOut : map.getAll()) {// 广播用户下线消息
					offOut.setMessage(offObject1);
				}
				break;
		  	case FriendDelete:
				// 给用户回复消息
				dao.DeleteFriend((User) read_tranObject.getObject()
				              ,read_tranObject.getFromUser());
				break;
